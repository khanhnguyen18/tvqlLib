    ...
    public partial class FrmMercLost : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void Txt_TransNum_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                ...
                case Keys.F5:
                    Class7.smethod_16("select trans_num as 'Số giao dịch', goods_id AS 'M\x00e3 h\x00e0ng', exp_id AS 'Nơi xuất', qty AS 'Số lượng', amount AS 'Tổng tiền', Status from tblTransaction where tran_date = '" + Class7.smethod_19() + "' and trans_code = '" + this.string_2 + "' order by trans_num ", this.oleDbConnection_0);
